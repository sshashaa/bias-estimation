...
best_subset<- function(data_corr,k){
  ...
  ga_postfit <- function(object, ...){
    # this condition checks the optimality gap
      ...
      if(abs((object@fitness[best_id]-mean(object@fitness[-best_id]))/mean(object@fitness[-best_id]))<0.5){#sqrt(var(object@fitness))/abs(mean(object@fitness))<2.5){
        print(abs((object@fitness[best_id]-mean(object@fitness[-best_id]))/mean(object@fitness[-best_id])))
      # selecting the worst case in all other cases.
        while(max(colSums(object@sampSize))<60){
          sorted_fit= (object@popSize+1) -rank(object@fitness, ties.method = "min")
          best_id= sorted_fit[1]
      ...
      #A2_tst= sum(object@sampSize[rl,-best_id]^2/object@fitvar[rl,-best_id])
      ...
      if(A1_tst<A2_tst){
        ...
        inp_ind= which.max(R_b1)
        des_ind= best_id
      }else{
        des_ind=c(1:object@popSize)[-best_id][which.min(R_b2)]
        inp_ind= rl[des_ind]
      }
      print(c(inp_ind,des_ind))
      ...
      object@fitvar[inp_ind,des_ind]= (object@fitvar[inp_ind,des_ind]*object@sampSize[inp_ind,des_ind]+delta*new_out[2])/(object@sampSize[inp_ind,des_ind]+delta)
...
r<-sample(1:nrow(full),floor(nrow(full)*0.15)) # we used 50% for test and 50% for training
data_corr<-full[r,c(1:30,ncol(full))]
test_data<-full[-r,]
OOS<-array(0,dim =6,
