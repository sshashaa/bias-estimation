...
best_subset<- function(data_corr,Exp_Name,dt_name,k){
  require('caret')
  require('GA')
  ...
  ga_postfit <- function(object, ...){
    ...
    while((max(colSums(object@sampSize))<60) & (est_err>(abs(best_sol)*opt_gap/5))){
      print(paste0("The opt gap is: ", round(opt_gap,2), " the est err is: ", round(est_err,2)))
      sorted_fit= (object@popSize+1) -rank(object@fitness, ties.method = "min")
      best_id= sorted_fit[1]
  ...
  fitness_Debiased_fixed <- function(string) {
    ...
    for (j in 1:B1) {
      set.seed(random_seed_train[j])
      tst1[,j]<- train[,j] #sample(1:nrow(data_tmp),q,replace = T)
  ...
  fitness_Debiased_adap <- function(string,delta=1,b1_ind) {
    ...
    rr=0.5
    rr2=0.5
    q=nrow(train)#floor(min(nrow(data_tmp),max(ncol(data_tmp)/((0.632*rr2)^3), floor(nrow(data_tmp)*rr)))) 
    ##
  ...
  saveRDS(garf,paste0("/home/kvahdat/Data/ch4/GA_",Exp_Name,"_",dt_name,"_",k,".rds"))
  ### End time
  ptm6<-Sys.time()-ptm
...
datasets = c("SmallDF", "LargeDF", "SmallDF_Corr", "LargelDF_Corr")
